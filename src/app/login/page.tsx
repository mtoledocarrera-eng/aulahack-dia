...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Error al iniciar sesión con email y contraseña. Verifica tus datos.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="mb-10 text-center relative z-10 flex flex-col items-center">
        ...
        <p className="text-slate-400">Ingresa para continuar analizando tus datos escolares.</p>
      </div>
        ...
        <form onSubmit={handleEmailLogin} className="space-y-5">
          ...
          <div className="pt-2">
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-xl text-slate-200 font-semibold transition-all"
            >
          ...
          <div className="flex items-center gap-4 py-4">
            ...
            <span className="text-xs font-bold tracking-widest text-slate-600">O POR EMAIL</span>
            <div className="flex-1 h-px bg-slate-800"></div>
          </div>
          ...
          <button 
            type="submit" 
            disabled={isLoggingIn}
            className="w-full px-4 py-3 bg-brand-500 hover:bg-brand-400 text-white rounded-xl font-bold transition-all shadow-neon flex items-center justify-center gap-2"
          >
